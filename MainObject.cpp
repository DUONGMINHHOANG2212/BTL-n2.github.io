
#include "MainObject.h"

MainObject::MainObject()
{
    frame_ = 0;
    x_pos_ = 0;//vị trí nhân vật
    y_pos_ = 0;
    x_val_ = 0;//ấn phải trái sẽ di chuyển bao nhiêu
    y_val_ = 0;
    width_frame_ = 0;
    height_frame_ = 0;
    status_ = -1;//trạng thái
    input_type_.left = 0;
    input_type_.right = 0;
    input_type_.jump = 0;
    input_type_.down = 0;
    input_type_.up = 0;
    on_ground = false;
    map_x_ = 0;
    map_y_ = 0;
    comeback_time = 0;
    money_count = 0;
    NUM_DIE=0;
}

MainObject::~MainObject()
{

}

bool MainObject::LoadImg(std::string path, SDL_Renderer* screen)
{
    bool ret = BaseObject::LoadImg(path, screen);//tan dung lop cha

    if(ret==true)
    {
        width_frame_ = rect_.w/8;
        height_frame_= rect_.h;
    }
    return ret;
}

SDL_Rect MainObject::GetRectFrame()
{
    SDL_Rect rect;
    rect.x = rect_.x;
    rect.y = rect_.y;
    rect.w = width_frame_;
    rect.h = height_frame_;

    return rect;

}

void MainObject::set_clips()
{
    if(width_frame_>0 && height_frame_>0)
    {
        frame_clip_[0].x=0;
        frame_clip_[0].y=0;
        frame_clip_[0].w=width_frame_;
        frame_clip_[0].h=height_frame_;

        frame_clip_[1].x=width_frame_;
        frame_clip_[1].y=0;
        frame_clip_[1].w=width_frame_;
        frame_clip_[1].h=height_frame_;

        frame_clip_[2].x=2*width_frame_;
        frame_clip_[2].y=0;
        frame_clip_[2].w=width_frame_;
        frame_clip_[2].h=height_frame_;

        frame_clip_[3].x=3*width_frame_;
        frame_clip_[3].y=0;
        frame_clip_[3].w=width_frame_;
        frame_clip_[3].h=height_frame_;

        frame_clip_[4].x=4*width_frame_;
        frame_clip_[4].y=0;
        frame_clip_[4].w=width_frame_;
        frame_clip_[4].h=height_frame_;

        frame_clip_[5].x=5*width_frame_;
        frame_clip_[5].y=0;
        frame_clip_[5].w=width_frame_;
        frame_clip_[5].h=height_frame_;

        frame_clip_[6].x=6*width_frame_;
        frame_clip_[6].y=0;
        frame_clip_[6].w=width_frame_;
        frame_clip_[6].h=height_frame_;

        frame_clip_[7].x=7*width_frame_;
        frame_clip_[7].y=0;
        frame_clip_[7].w=width_frame_;
        frame_clip_[7].h=height_frame_;
    }
}

void MainObject::Show(SDL_Renderer* des)
{
    if(on_ground == true)
    {
        if(status_ == WALK_LEFT)
        {
            LoadImg("image/player/p1/player_left.png",des);
        }
        else
        {
            LoadImg("image/player/p1/player_right.png",des);
        }
    }


    if(input_type_.left ==1 || input_type_.right == 1)
    {
        frame_++;
    }
    else
    {
        frame_ = 0;
    }
    if(frame_>=8)
    {
        frame_=0;
    }
    if(comeback_time == 0)
    {
    rect_.x = x_pos_ - map_x_;
    rect_.y = y_pos_ - map_y_;

    SDL_Rect* current_clip = &frame_clip_[frame_];
    SDL_Rect renderQuad={rect_.x, rect_.y, width_frame_, height_frame_};
    SDL_RenderCopy(des, p_object_, current_clip, &renderQuad);
    }
}

void MainObject::HandelInputAction(SDL_Event events, SDL_Renderer* screen,Mix_Chunk* g_sound[2])
{
    int left = 0;
    if(events.type == SDL_KEYDOWN)
    {
        switch(events.key.keysym.sym)
        {
        case SDLK_RIGHT:
            {   left = 0;
                status_ = WALK_RIGHT;
                input_type_.right = 1;
                input_type_.left=0;
                if(on_ground == true)
                {
                    LoadImg("image/player/p1/player_right.png", screen);
                }
                else
                {
                    LoadImg("image/player/p1/jum_right.png", screen);
                }
            }
            break;
        case SDLK_LEFT:
            {
                left = 1;
               status_ = WALK_LEFT;
               input_type_.left = 1;
               input_type_.right =0;
               if(on_ground == true)
                {
                    LoadImg("image/player/p1/player_left.png", screen);
                }
                else
                {
                    LoadImg("image/player/p1/jum_left.png", screen);
                }
            }
            break;
        }
    }
    else if (events.type == SDL_KEYUP)
    {
         switch(events.key.keysym.sym)
        {
        case SDLK_RIGHT:
            {
                input_type_.right = 0;
            }
            break;
        case SDLK_LEFT:
            {
                input_type_.left = 0;
            }
            break;
        }
    }

    if(events.type == SDL_KEYDOWN)
    {
        if(events.key.keysym.sym == SDLK_UP)
        {
            if(status_==WALK_LEFT)
            {
                 LoadImg("image/player/p1/jum_left.png", screen);
            }
            else
            {
                 LoadImg("image/player/p1/jum_right.png", screen);
            }
            input_type_.jump=1;
        }

    }
    if(events.type == SDL_MOUSEBUTTONDOWN)
    {
        if(events.button.button == SDL_BUTTON_LEFT)
        {
            BulletObj* p_bullet = new BulletObj();
            if(money_count<20)
            {
                p_bullet-> LoadImg("image/bullet/b4.png", screen);
                Mix_PlayChannel(-1,g_sound[0],0);//am thanh khi ban
            }
            else if(money_count<40)
            {
                p_bullet-> LoadImg("image/bullet/b6.png", screen);
                 Mix_PlayChannel(-1,g_sound[0],0);//am thanh khi ban
            }
            else if(money_count<60)
            {
                p_bullet-> LoadImg("image/bullet/la1.png", screen);
                 Mix_PlayChannel(-1,g_sound[0],0);//am thanh khi ban
            }
            else
            {
                p_bullet-> LoadImg("image/bullet/la3.png", screen);
                 Mix_PlayChannel(-1,g_sound[0],0);//am thanh khi ban
            }

            if(status_ == WALK_LEFT)
            {
                p_bullet->set_bullet_direct(BulletObj::DIR_LEFT);
                 p_bullet-> SetRect(this-> rect_.x, rect_.y +height_frame_*0.2 );
            }
            else
            {
                p_bullet->set_bullet_direct(BulletObj::DIR_RIGHT);
                 p_bullet-> SetRect(this-> rect_.x + width_frame_ - 20, rect_.y +height_frame_*0.2 );
            }

            p_bullet-> set_x_val(BULLET_SPEED);
            p_bullet -> set_is_move(true);

            p_bullet_list.push_back(p_bullet);
        }
    }
}

void MainObject::HandleBullet(SDL_Renderer* des)
{
    for(int i = 0; i< p_bullet_list.size(); i++)
    {
        BulletObj* p_bullet = p_bullet_list.at(i);
        if(p_bullet != NULL)
        {
            if(p_bullet-> get_is_move()==true)
            {
                p_bullet-> HandleMove(SCREEN_WIDTH, SCREEN_HEIGHT);
                p_bullet-> Render(des);

            }
            else
            {
                p_bullet_list.erase(p_bullet_list.begin()+i);
                if(p_bullet!= NULL)
                {
                    delete p_bullet;
                    p_bullet = NULL;
                }
            }
        }
    }
}
//va chạm đạn của nv và quái
void MainObject::RemoveBullet(const int& index)
{
    int siz = p_bullet_list.size();
    if(siz > 0 && index <siz)
    {
        BulletObj* p_bullet = p_bullet_list.at(index);
        p_bullet_list.erase(p_bullet_list.begin()+index);

        if(p_bullet)
        {
            delete p_bullet;
            p_bullet = NULL;
            money_count++;//giết quái cộng điểm
        }
    }
}

void MainObject::DoPalyer(Map& map_data)
{
    if(comeback_time == 0)
    {
    x_val_ = 0;
    y_val_+= GRAVITY_SPEED;

    if(y_val_>= MAX_FALL_SPEED)
    {
        y_val_ = MAX_FALL_SPEED;
    }

    if(input_type_.left==1)
    {
        x_val_ -=PLAYER_SPEED;
    }
    else if(input_type_.right ==1)
    {
        x_val_ +=PLAYER_SPEED;
    }

    if(input_type_.jump ==1)
    {
        if(on_ground == true)
        {
            y_val_ = -PLAYER_JUMP_VAL;
        }
        on_ground = false;
        input_type_.jump = 0;
    }

    CheckToMap(map_data);
    CenterEntityOnMap(map_data);
    }
    if(comeback_time >0)
    {
        comeback_time --;
        if(comeback_time == 0)
        {
            NUM_DIE++;
            if(x_pos_ > 6* TILE_SIZE)
            {
               x_pos_ -= 6*TILE_SIZE;
            }
            else
            {
                x_pos_ = 0;
            }

            y_pos_ = 0;
            x_val_ = 0;
            y_val_ = 0;
        }
    }
}



void MainObject::CenterEntityOnMap(Map& map_data)
{
    map_data.start_x_ = x_pos_ -(SCREEN_WIDTH/2);//qua 1/2 man hinh thi man hinh cuon theo
    if(map_data.start_x_ <0)
    {
        map_data.start_x_ = 0;
    }
    else if(map_data.start_x_ +SCREEN_WIDTH >= map_data.max_x_)
    {
        map_data.start_x_=map_data.max_x_-SCREEN_WIDTH;
    }

    map_data.start_y_ = y_pos_ -(SCREEN_HEIGHT/2);
    if(map_data.start_y_<0)
    {
        map_data.start_y_=0;
    }
    else if(map_data.start_y_ + SCREEN_HEIGHT >= map_data.max_y_)
    {
        map_data.start_y_=map_data.max_y_-SCREEN_HEIGHT;
    }
}

void MainObject::CheckToMap(Map& map_data)
{
    //gioi han kiem tra tu a den b theo chieu x
    int x1 = 0;
    int x2 = 0;

    int y1 = 0;
    int y2 = 0; //kiem tra theo chieu doc
    bool check_eat = false;



    // check chieu ngang
    int height_min = height_frame_ <TILE_SIZE ? height_frame_ :TILE_SIZE;

    x1 = (x_pos_ + x_val_)/TILE_SIZE;
    x2 = (x_pos_ + x_val_ + width_frame_-1)/TILE_SIZE;

    y1 = (y_pos_ )/TILE_SIZE;
    y2 = (y_pos_ +  height_min -1)/TILE_SIZE;


    if(map_data.tile[y1][x2]==VAT_PHAM||map_data.tile[y1][x1]==VAT_PHAM||map_data.tile[y2][x1]==VAT_PHAM||map_data.tile[y2][x2]==VAT_PHAM)
    {
        check_eat=true;
    }

    if(x1 >= 0 && x2<MAX_MAP_X && y1 >=0 && y2< MAX_MAP_Y)
    {
        if(x_val_ > 0) //player di chuyen sang phai
        {

            int val1 = map_data.tile[y1][x2];
            int val2 = map_data.tile[y2][x2];

            if(val1 == VAT_PHAM ||val2 == VAT_PHAM)
            {
                map_data.tile[y1][x2] == 0;
                map_data.tile[y2][x2] == 0;
            }
            else
            {
                if(val1 != BLANK_TILE || val2 != BLANK_TILE)
                {
                    // va cham map
                    x_pos_ = x2*TILE_SIZE;
                    x_pos_ -= width_frame_ +1;
                    x_val_ = 0;
                }
            }
        }
        else if(x_val_ <0)
            {
                int val1 = map_data.tile[y1][x1];
                int val2 = map_data.tile[y2][x1];

                if(val1 == VAT_PHAM ||val2 == VAT_PHAM)
                {
                    map_data.tile[y1][x1] == 0;
                    map_data.tile[y2][x1] == 0;

                }
                else
                {
                    if(val1 != BLANK_TILE || val2 != BLANK_TILE)
                    {
                        x_pos_ = (x1+1)*TILE_SIZE;
                        x_val_ =0;

                    }
                }

            }

    }

    // check chieu doc

    int width_min = width_frame_ <TILE_SIZE ? width_frame_ : TILE_SIZE;
    x1 = (x_pos_)/TILE_SIZE;
    x2 = (x_pos_ + width_min)/TILE_SIZE;

    y1 = (y_pos_ +y_val_)/TILE_SIZE;
    y2 = (y_pos_ + y_val_ + height_frame_ -1)/TILE_SIZE;

    if(map_data.tile[y1][x2]==VAT_PHAM||map_data.tile[y1][x1]==VAT_PHAM||map_data.tile[y2][x1]==VAT_PHAM||map_data.tile[y2][x2]==VAT_PHAM)
    {
        check_eat=true;
    }


    if(x1>=0 && x2< MAX_MAP_X && y1>=0 && y2 < MAX_MAP_Y)
    {
        if(y_val_ >0)
        {
            int val1 = map_data.tile[y2][x1];
            int val2 = map_data.tile[y2][x2];

            if(val1 == VAT_PHAM || val2 == VAT_PHAM)
            {
                map_data.tile[y2][x1] = 0;
                map_data.tile[y2][x2] = 0;

            }
            else
            {
                if(val1 != BLANK_TILE || val2 != BLANK_TILE)
                {
                    y_pos_ = y2*TILE_SIZE;
                    y_pos_ -=height_frame_ +1;
                    y_val_ = 0;
                    on_ground = true;
                }
            }
        }
        else if (y_val_ <0)
        {
            int val1 = map_data.tile[y1][x1];
            int val2 =  map_data.tile[y1][x2];
            if(val1 == VAT_PHAM || val2 == VAT_PHAM)
            {
                map_data.tile[y1][x1] = 0;
                map_data.tile[y1][x2] = 0;

            }
            else
            {
                 if(val1 != BLANK_TILE || val2 != BLANK_TILE)
                {
                    y_pos_ =(y1 +1)*TILE_SIZE;
                    y_val_=0;

                }
            }

        }
    }
    if(check_eat)
    {
        money_count++;
    }
    x_pos_ +=x_val_;
    y_pos_ +=y_val_;


    if(x_pos_ <0)
    {
        x_pos_ = 0;
    }
    else if(x_pos_ +width_frame_ > map_data.max_x_)
    {
        x_pos_ = map_data.max_x_ - width_frame_ -1;
    }

    // xu ly roi vuc
    if(y_pos_ > map_data.max_y_)
    {
        comeback_time = REVIVE;
    }

}




